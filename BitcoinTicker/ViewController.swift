        ...
        Alamofire.request(url, method: .get)
            .responseJSON { response in
                if response.result.isSuccess {
                    ...
                    print("BitCoin Ticker Request Successfully done.")
                    let tickerJSON : JSON = JSON(response.result.value!)

                    self.updateBitcoinTickerData(json: tickerJSON)
...
//    /***************************************************************/
    ...
    func updateBitcoinTickerData(json : JSON) {
        ...
        if let tempResult = json["ask"].double {
            self.bitcoinPriceLabel.text = "\(Int(tempResult.rounded())) \(currencySymbols[selectedRow])"
        } else {
            self.bitcoinPriceLabel.text = "Network Issue"
        }
